...
function addFormatToken (token, padded, ordinal, callback) {
    ...
    if (padded) {
        formatTokenFunctions[padded[0]] = function () {
            return zeroFill(func.apply(arguments), padded[1], padded[2]);
        };
    }
    if (ordinal) {
        formatTokenFunctions[ordinal] = function () {
            return this.localeData().ordinal(func.apply(arguments), token);
        };
    }
}
...
addFormatToken('MMM', 0, 0, function (format) {
    return this.localeData().monthsShort(format);
});

addFormatToken('MMMM', 0, 0, function (format) {
    return this.localeData().months(format);
});
...
function localeMonthsParse (monthName, format, strict) {
    ...
    if (this._monthsParseExact) {
        return handleStrictParse.call(monthName, format, strict);
    }
...
function monthsShortRegex (isStrict) {
    if (this._monthsParseExact) {
        ...
        if (!hasOwnProp('_monthsShortRegex')) {
...
addFormatToken('dd', 0, 0, function (format) {
    return this.localeData().weekdaysMin(format);
});

addFormatToken('ddd', 0, 0, function (format) {
    return this.localeData().weekdaysShort(format);
});

addFormatToken('dddd', 0, 0, function (format) {
    return this.localeData().weekdays(format);
});
...
function localeWeekdaysParse (weekdayName, format, strict) {
    ...
    if (this._weekdaysParseExact) {
        return handleStrictParse$1.call(weekdayName, format, strict);
    }
    ...
    if (input != null) {
        ...
        if (offset !== input) {
            if (!keepLocalTime || this._changeInProgress) {
                addSubtract(createDuration(input - offset, 'm'), 1, false);
...
function parseIso (inp, sign) {
    // We'd normally use ~~inp for but unfortunately it also
...
function calendar$1 (time, formats) {
    ...
    var now = time || createLocal(),
        ...
        format = hooks.calendarFormat(sod) || 'sameElse';

    var output = formats && (isFunction(formats[format]) ? formats[format].call(now) : formats[format]);

    return this.format(output || this.localeData().calendar(format, createLocal(now)));
}
...
function format (inputString) {
    ...
    var output = formatMoment(inputString);
...
function from (time, withoutSuffix) {
    if (this.isValid() &&
        ...
        return createDuration({to: from: time}).locale(this.locale()).humanize(!withoutSuffix);
    } else {
...
function to (time, withoutSuffix) {
    if (this.isValid() &&
            ((isMoment(time) && time.isValid()) ||
             createLocal(time).isValid())) {
        return createDuration({from: to: time}).locale(this.locale()).humanize(!withoutSuffix);
    } else {
        return this.localeData().invalidDate();
    }
}
...
function getSetWeekYearHelper(input, week, weekday, dow, doy) {
    var weeksTarget;
    if (input == null) {
        return weekOfYear(dow, doy).year;
    } else {
        weeksTarget = weeksInYear(input, dow, doy);
        if (week > weeksTarget) {
            week = weeksTarget;
        }
        return setWeekAll.call(input, week, weekday, dow, doy);
    }
}
...
function add$1 (input, value) {
    return addSubtract$1(input, value, 1);
}
...
function humanize (withSuffix) {
    ...
    var output = relativeTime$1(!withSuffix, locale);

    if (withSuffix) {
        output = locale.pastFuture(+output);
    }
...
module.exports = function(Chart) {
	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		removeHoverStyle: function(arc) {
			Chart.DatasetController.prototype.removeHoverStyle.call(arc, this.chart.options.elements.arc);
		},
	...
	helpers.extend(Chart.prototype, /** @lends Chart */ {
		...
		*/
		...
		updateLayout: function() {
			...
			Chart.layoutService.update(this.width, this.height);
		...
		getElementAtEvent: function(e) {
			return Chart.Interaction.modes.single(e);
		},

		getElementsAtEvent: function(e) {
			return Chart.Interaction.modes.label(e, {intersect: true});
		},

		getElementsAtXAxis: function(e) {
			return Chart.Interaction.modes['x-axis'](e, {intersect: true});
		},

		getElementsAtEventForMode: function(e, mode, options) {
			var method = Chart.Interaction.modes[mode];
			if (typeof method === 'function') {
				return method(e, options);
			}
		...
		getDatasetAtEvent: function(e) {
			return Chart.Interaction.modes.dataset(e, {intersect: true});
		},
...
module.exports = function(Chart) {
	...
	Chart.Tooltip = Chart.Element.extend({
		...
		getBeforeBody: function() {
			var lines = this._options.callbacks.beforeBody.apply(arguments);
		...
		getAfterBody: function() {
			var lines = this._options.callbacks.afterBody.apply(arguments);
			return helpers.isArray(lines) ? lines : lines !== undefined ? [lines] : [];
		},
		...
		update: function(changed) {
			...
			if (active.length) {
				...
				tooltipSize = getTooltipSize(model);
				alignment = determineAlignment(tooltipSize);
